<?php 
/**
 * Services section Template
 * @package customtheme
 */
?>
<main class="bg-white py-5">
    <div class="container">
<div class="row g-4 justify-content-center services-grid">
      <div class="col-lg-4 col-md-6 d-flex justify-content-center">
        <div class="card h-100 w-100" data-tilt>
          <h2>UI/UX Design</h2>
          <p>Elevate your user experience with expert UI/UX design—intuitive, beautiful, and conversion-focused.</p>
          <a href="mailto:leelasonscorporation@gmail.com" class="btn" target="_blank">Read More</a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 d-flex justify-content-center d-none d-md-block">
        <div class="card h-100 w-100" data-tilt>
          <h2>Web Development</h2>
          <p>Robust, scalable web apps built with modern stacks and best practices for performance and security.</p>
          <a href="mailto:leelasonscorporation@gmail.com" class="btn" target="_blank">Read More</a>
        </div>
      </div>

      <div class="col-lg-4 col-md-6 d-flex justify-content-center d-none d-lg-block">
        <div class="card h-100 w-100" data-tilt>
          <h2>Digital Marketing</h2>
          <p>Data-driven marketing, SEO and social strategies to grow traffic and conversions.</p>
          <a href="mailto:leelasonscorporation@gmail.com" class="btn" target="_blank">Read More</a>
        </div>
      </div>
</div>
    </div>
    </main>

    <section class="services-container p-lg-5 p-2">
      <div class="container">
        <div class="row align-items-center justify-content-center">
          <div class="col-lg-4 col-md-0 d-none d-lg-block">
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
            <div class="strain"></div>
          </div>

          <div class="col-lg-8 col-md-12">
        <p class="h2 w-100">Leela infotech delivers technology solutions that combine engineering precision, strategic clarity, and deep empathy. We collaborate closely with clients to build digital platforms, custom apps, and integrated systems that align with business goals and drive measurable results.</p>
          </div>
        </div>
      </div>
    </section>